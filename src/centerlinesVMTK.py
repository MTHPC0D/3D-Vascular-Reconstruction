...
def main():
    """Fonction principale d'extraction des lignes centrales"""
    try:
        ...
        preserve_main_structure = os.environ.get("CENTERLINES_PRESERVE_MAIN", "True").lower() == "true"        # Convertir en chemins absolus si nécessaire
        if not os.path.isabs(stl_file):
            stl_file = os.path.abspath(stl_file)
        if not os.path.isabs(out_vtp):
            out_vtp = os.path.abspath(out_vtp)

        # Créer le répertoire de sortie si nécessaire
        os.makedirs(os.path.dirname(out_vtp), exist_ok=True)
        
        print(f"[INFO] Fichier STL: {stl_file}")
        print(f"[INFO] Sortie VTP: {out_vtp}")

        # Vérifier que le fichier STL existe
        if not os.path.exists(stl_file):
            raise FileNotFoundError(f"Fichier STL non trouvé: {stl_file}")
        ...
        if stl_poly.GetNumberOfPoints() == 0:
            raise ValueError("Le fichier STL semble vide ou corrompu")
        ...
        print("[SUCCESS] Volume voxelise et rempli")
        ...
        def world(v):
            """Conversion voxel -> coordonnees monde"""
        ...
        if do_smooth:
            ...
            print("[SUCCESS] Lissage applique")        # 11. Export final
        writer = vtk.vtkXMLPolyDataWriter()
        writer.SetFileName(out_vtp)
        writer.SetInputData(centerlines)
        writer.Write()

        print(f"[SUCCESS] Lignes centrales : {centerlines.GetNumberOfPoints()} points, {centerlines.GetNumberOfLines()} lignes")
        print(f"[SUCCESS] Fichier sauvegarde: {out_vtp}")
        
    except Exception as e:
        print(f"[ERROR] Erreur lors de l'extraction des lignes centrales: {str(e)}")
        sys.exit(1)
